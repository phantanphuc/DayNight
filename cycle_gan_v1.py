...
class cycleGAN(object):
    ...
    def _build_net(self):
        ...
        # hsv_x = tf.image.rgb_to_hsv(self.x_imgs)
        # hsv_y = tf.image.rgb_to_hsv(self.y_imgs)
        ...
        #     gt = tf.greater(self.x_imgs, i)
        #     leq = tf.less_equal(self.x_imgs, i + bin_size)

        #     node = tf.reduce_sum(tf.cast(tf.logical_and(gt, leq), tf.float32), axis=(1, 2))
        #     hist_entries_x.append(node)


        # for idx, i in enumerate(np.arange(-1, 1, bin_size)):
        #     gt = tf.greater(self.y_imgs, i)
        #     leq = tf.less_equal(self.y_imgs, i + bin_size)

        #     node = tf.reduce_sum(tf.cast(tf.logical_and(gt, leq), tf.float32), axis=(1, 2))
        #     hist_entries_y.append(node)


        # hist_x = tf.stack(hist_entries_x)
        # hist_x = tf.transpose(hist_x, perm=[1, 2, 0])
        # hist_x = tf.reshape(hist_x, shape=(-1, 3 * bin_count))
        # hist_x = tf.tile(hist_x, [1, self.image_size[0] * self.image_size[1]])
        # hist_x = tf.reshape(hist_x, shape=(-1, self.image_size[0], self.image_size[1], 3 * bin_count))
        # self.histogram_x = hist_x / (self.image_size[0] * self.image_size[1])


        # hist_y = tf.stack(hist_entries_y)
        # hist_y = tf.transpose(hist_y, perm=[1, 2, 0])
        # hist_y = tf.reshape(hist_y, shape=(-1, 3 * bin_count))
        # hist_y = tf.tile(hist_y, [1, self.image_size[0] * self.image_size[1]])
        # hist_y = tf.reshape(hist_y, shape=(-1, self.image_size[0], self.image_size[1], 3 * bin_count))
        # self.histogram_y = hist_y / (self.image_size[0] * self.image_size[1])

        self.histogram_x = self.createHistogram(self.x_imgs, self.hist_bin_size)
        self.histogram_y = self.createHistogram(self.y_imgs, self.hist_bin_size)
        ...
        # HIST LOSS
        self.histogram_fake_x = self.createHistogram(self.fake_x_imgs, self.hist_bin_size)
        self.histogram_fake_y = self.createHistogram(self.fake_y_imgs, self.hist_bin_size)
        ...
        self.G_loss = self.G_gen_loss + cycle_loss + hist_loss_y / 100000
        self.F_loss = self.F_gen_loss + cycle_loss + hist_loss_x / 100000
