...
def dense(x, output_dim, name='fc'):
    # x: batch 382
    with tf.variable_scope(name):
        fc1_W = tf.get_variable('fc_w', (x.get_shape()[-1], output_dim))
        fc1_b = tf.get_variable('fc_b', output_dim)
        fc1   = tf.nn.bias_add(tf.matmul(x, fc1_W), fc1_b)
